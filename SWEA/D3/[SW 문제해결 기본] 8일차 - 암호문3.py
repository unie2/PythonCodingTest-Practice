...
2. order 리스트의 요소를 순서대로 하나씩 확인하며, 만약 해당 값이 'I' 혹은 'D' 혹은 'A'일 경우 type을 해당 문자로 갱신하고, pos와 cnt를 -1로 설정한다.
...
3. 해당 값이 숫자라면 아래와 같은 작업을 수행한다.
  ...
  - 만약 type이 'A'이고 cnt가 -1이라면, 현재 값이 추가할 값의 개수를 의미하므로 cnt에 해당 값(정수형으로 변환)을 할당한다.
  - 만약 type이 'A'이고 cnt가 -1이 아니라면, 해당 값(정수형으로 변환)을 data 리스트 뒤쪽에 추가한다.
